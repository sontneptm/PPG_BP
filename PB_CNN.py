...
model.summary()
model.fit(xd, yd, epochs=EPOCH, verbose=2, callbacks=[cp_callback])

pd = model.predict(test_data)
...
for i in range(len(xd)):
    ...
    print("실제값 : ", yd[i],"\t", "예측값 :", pd[i])

print(r2_score(yd, pd))

sys_list = []
dia_list = []
for i in range(len(yv)) :
    sys_list.append(pd[i][0]-yd[i][0])
    dia_list.append(pd[i][1]-yd[i][1])

print("SBP MAE:",mean_absolute_error(pd[:,0:1], yd[:,0:1]))
print("DBP MAE:",mean_absolute_error(pd[:,1:2], yd[:,1:2]))
